...
| 2 | API host | **SWA managed Functions (Python)**, same origin under `/api` | $0, no CORS, one deploy command; fallback to a standalone Function App is non-breaking if managed limits ever bite. |
...
| 7 | Layout | 3-column fixed viewport: collapsible lead rail (picker + filters) + Deal pane + Activity pane; only panes scroll | Requirements omit how a lead gets selected; rail is the email-client pattern, no extra screens. 2 screens total (login, workspace). |
...
**To proceed:** reply with approval (and your call on decision #4's Entra alternative if you want it).
